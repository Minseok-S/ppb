...
// ════════════════════════════════════════
const S = {
  companyName:'', serviceName:'', effectiveDate:'',
  collectNoConsent:[], collectConsent:[], collectAuto:[],
  child:'no', childMethod:'',
  retention:{contract:true,dispute:true,ad:true,log:true},
  customRetention:[],
  destroy:{electronic:true,paper:true},
  thirdParty:'no', tpConsent:[], tpLegal:[],
  delegate:'no', dlItems:[],
  overseas:'no', otItems:[],
  security:{s_plan:true,s_edu:true,s_org:false,s_access:true,s_encrypt:true,s_sec:true,s_log:false,s_vuln:false,s_phys:true,s_media:false,s_isms:false},
  cookie:'yes',
  browser:{b_chrome:true,b_edge:true,b_safari:false,b_samsung:false},
  behavioral:'no', bhItems:[],
  rights:{r_written:true,r_phone:true,r_email:true,r_fax:false,r_web:false},
  rightsPath:'', mydata:'no',
  cpoName:'', cpoTitle:'', cpoPhone:'', cpoEmail:'',
  depts:[{name:'',phone:'',email:''}],
  agency:{ag_kopico:true,ag_kisa:true,ag_spo:true,ag_police:true},
  addUsage:'no', addUsageText:'',
  sensitive:'no', sensitiveText:'',
  pseudonym:'no', pseudonymText:'',
  autoDecision:'no', autoDecisionText:'',
  domAgent:'no', daName:'', daPhone:'', daAddr:'', daEmail:''
};
...
let curStep = 1;
const TOTAL = 12;
const stepLabels = ['기본 정보','수집 항목','파기','제3자 제공','위탁','국외이전','안전조치','쿠키','행태정보','권리행사','책임자','추가항목'];

function goStep(n){
  document.getElementById('step'+curStep).classList.remove('active');
  document.querySelector('[data-step="'+curStep+'"]').classList.remove('active');
  curStep = n;
  document.getElementById('step'+curStep).classList.add('active');
  document.querySelector('[data-step="'+curStep+'"]').classList.add('active');
  document.getElementById('progressFill').style.width = Math.round(curStep/TOTAL*100)+'%';
  document.getElementById('progressText').textContent = 'STEP '+curStep+'/'+TOTAL+' · '+stepLabels[curStep-1];
  document.getElementById('prevBtn').style.display = curStep>1?'flex':'none';
  document.getElementById('nextBtn').style.display = curStep<TOTAL?'flex':'none';
}
function nextStep(){ if(curStep<TOTAL){ document.querySelector('[data-step="'+curStep+'"]').classList.add('done'); goStep(curStep+1); }}
...
function toggleItem(el, group){
  ...
  const maps = {retention:S.retention, destroy:S.destroy, security:S.security, browser:S.browser, rights:S.rights, agency:S.agency};
...
const basisMap = Object.fromEntries(basisOpts.map(o=>[o.v, '「개인정보 보호법」 '+o.l]));
...
function addCollect(type){
  ...
  div.className='card-item'; div.id=cid;
  div.innerHTML=`
...
function syncCollect(type){
  ...
  document.querySelectorAll('#collect'+cap(type)+' .card-item').forEach(d=>{
    ...
    arr.push({basis:g('basis')||'consent',category:g('category'),purpose:g('purpose'),items:g('items'),retention:g('retention')});
  });
...
function addCustomRetention(){
  const id='cr_'+Date.now(), c=document.getElementById('customRetention');
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  d.innerHTML=`
...
function removeAndSyncCR(id){document.getElementById(id)?.remove();updatePreview();}
function syncCR(){
  S.customRetention=[];
  document.querySelectorAll('#customRetention .card-item').forEach(d=>{
    const g=f=>d.querySelector('[data-field="'+f+'"]')?.value||'';
    S.customRetention.push({label:g('label'),basis:g('basis'),period:g('period')});
  });
  // state updated
}
...
// ════════════════════════════════════════
function addTP(type){
  const id='tp_'+type+'_'+Date.now(), c=document.getElementById('tp'+cap(type));
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  const legalBasisHtml = type==='legal'?`<div class="field-group"><label class="field-label">제공 법적 근거</label><input type="text" data-field="basis" placeholder="예: 개인정보보호법 제17조①2호, 소득세법 제165조" oninput="syncTP();updatePreview()"></div>`:'';
  d.innerHTML=`
...
function removeAndSyncTP(id){document.getElementById(id)?.remove();syncTP();updatePreview();}
function syncTP(){
  ['Consent','Legal'].forEach(t=>{
    S['tp'+t]=[];
    document.querySelectorAll('#tp'+t+' .card-item').forEach(d=>{
      const g=f=>d.querySelector('[data-field="'+f+'"]')?.value||'';
      S['tp'+t].push({basis:g('basis'),receiver:g('receiver'),purpose:g('purpose'),items:g('items'),retention:g('retention')});
    });
  });
  // state updated
}
...
function addDelegate(){
  const id='dl_'+Date.now(), c=document.getElementById('dlItems');
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  d.innerHTML=`
...
function removeAndSyncDL(id){document.getElementById(id)?.remove();syncDL();updatePreview();}
function syncDL(){
  S.dlItems=[];
...
function addOverseas(){
  const id='ot_'+Date.now(), c=document.getElementById('otItems');
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  d.innerHTML=`
...
function removeAndSyncOT(id){document.getElementById(id)?.remove();syncOT();}
function syncOT(){
  S.otItems=[];
  document.querySelectorAll('#otItems .card-item').forEach(d=>{
    const g=f=>d.querySelector('[data-field="'+f+'"]')?.value||'';
    S.otItems.push({receiver:g('receiver'),country:g('country'),items:g('items'),purpose:g('purpose'),method:g('method'),retention:g('retention')});
  });
  // state updated
}
...
function addBehavioral(){
  const id='bh_'+Date.now(), c=document.getElementById('bhItems');
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  d.innerHTML=`
...
function removeAndSyncBH(id){document.getElementById(id)?.remove();syncBH();updatePreview();}
function syncBH(){
  S.bhItems=[];
  document.querySelectorAll('#bhItems .card-item').forEach(d=>{
    const g=f=>d.querySelector('[data-field="'+f+'"]')?.value||'';
    S.bhItems.push({items:g('items'),purpose:g('purpose'),method:g('method'),retention:g('retention')});
  });
  // state updated
}
...
// ════════════════════════════════════════
function addDept(){
  const id='dept_'+Date.now(), c=document.getElementById('deptItems');
  const d=document.createElement('div'); d.className='card-item'; d.id=id;
  d.innerHTML=`
...
function removeAndSyncDept(id){document.getElementById(id)?.remove();syncDepts();updatePreview();}
function syncDepts(){
  S.depts=[];
  // default row
  S.depts.push({
...
// ════════════════════════════════════════
function readFields(){
  ['companyName','serviceName','effectiveDate','childMethod',
   'cpoName','cpoTitle','cpoPhone','cpoEmail',
   'rightsPath','addUsageText','sensitiveText','pseudonymText','autoDecisionText',
   'daName','daPhone','daAddr','daEmail'].forEach(id=>{
...
function updatePreview(){
  ...
  document.querySelectorAll('#previewContent .pp-toc-link').forEach(a=>{
    a.addEventListener('click',function(e){
      ...
      const target = document.getElementById('previewContent').querySelector('#'+tid);
      if(target&&panel){
        const pr=panel.getBoundingClientRect(), tr=target.getBoundingClientRect();
        panel.scrollTo({top:panel.scrollTop+(tr.top-pr.top)-20,behavior:'smooth'});
      }
    });
  });
}
...
function computeSpans(arr, keyFn){
  const n=arr.length, span=new Array(n).fill(1), skip=new Array(n).fill(false);
  let i=0;
  while(i<n){
    const val=keyFn(i);
    if(!val){i++;continue;}
...
function buildCollectTable(arr, showBasis){
  if(!arr.length||!arr.some(r=>r.category||r.purpose||r.items)) return '<p style="color:#aaa;font-style:italic;font-size:12px;">← 항목을 추가해 주세요</p>';
  const bs=showBasis?computeSpans(arr,i=>arr[i].basis||''):{span:[],skip:[]};
  ...
  let rows=arr.map((r,i)=>{
    const bCell = showBasis?(bs.skip[i]?'':
    ...
    const rCell = (showBasis&&!rs.skip[i])?
...
function buildPreview(){
  ...
  // Retention table
  const retMap = {
    contract:{label:'계약 또는 청약철회, 대금결제, 재화 등의 공급기록',basis:'전자상거래 등에서의 소비자보호에 관한 법률 제6조',period:'5년'},
    dispute:{label:'소비자의 불만 또는 분쟁처리에 관한 기록',basis:'전자상거래 등에서의 소비자보호에 관한 법률 제6조',period:'3년'},
    ad:{label:'표시·광고에 관한 기록',basis:'전자상거래 등에서의 소비자보호에 관한 법률 제6조',period:'6개월'},
    log:{label:'웹사이트 방문기록(로그기록, IP 등) 및 본인확인 기록',basis:'통신비밀보호법 시행령 제41조',period:'3개월'},
  };
  const activeRet = Object.keys(S.retention).filter(k=>S.retention[k]).map(k=>retMap[k]);
  const allRet = [...activeRet, ...(S.customRetention||[]).filter(r=>r.label)];

  // Security
  const secMap = {
    s_plan:'내부 관리계획 수립·시행',s_edu:'정기적 직원 교육',s_org:'전담 조직 운영',
    s_access:'개인정보처리시스템 접근 권한의 관리 및 접근통제시스템 설치',
    s_encrypt:'개인정보의 암호화',s_sec:'보안프로그램 설치 및 갱신',
    s_log:'접속기록의 보관 및 점검',s_vuln:'개인정보처리시스템 취약점 점검 및 보완',
    s_phys:'전산실, 자료보관실 등의 접근통제',s_media:'보조저장매체 반출·입 통제',
    s_isms:'국내 정보보호 관리체계(ISMS-P) 인증 취득',
  };
  const secMgmt=['s_plan','s_edu','s_org'].filter(k=>S.security[k]).map(k=>secMap[k]);
  const secTech=['s_access','s_encrypt','s_sec','s_log','s_vuln'].filter(k=>S.security[k]).map(k=>secMap[k]);
  const secPhys=['s_phys','s_media'].filter(k=>S.security[k]).map(k=>secMap[k]);
  const secExtra=['s_isms'].filter(k=>S.security[k]).map(k=>secMap[k]);

  // Rights methods
  const rMap={r_written:'서면',r_phone:'전화',r_email:'전자우편',r_fax:'팩스(FAX)',r_web:'인터넷'};
  const activeMethods=Object.keys(S.rights).filter(k=>S.rights[k]).map(k=>rMap[k]).join(', ');

  // Browsers
  const bMap={b_chrome:{n:'크롬(Chrome)',p:'웹브라우저 오른쪽 상단 ⋮ > 새 시크릿 창 (Ctrl+Shift+N)'},b_edge:{n:'엣지(Edge)',p:'웹브라우저 오른쪽 상단 … > 새 InPrivate 창 (Ctrl+Shift+N)'},b_safari:{n:'사파리(Safari)',p:'모바일 기기 설정 > 사파리(Safari) > 고급 > 모든 쿠키 차단'},b_samsung:{n:'삼성 인터넷',p:"모바일 브라우저 하단 '탭' 아이콘 > 비밀 모드 켜기 > 시작"}};
  const activeBrowsers=Object.keys(S.browser).filter(k=>S.browser[k]);

  // Agencies
  const agMap={ag_kopico:{n:'개인정보 분쟁조정위원회',u:'www.kopico.go.kr',t:'(국번없이) 1833-6972'},ag_kisa:{n:'개인정보침해 신고센터',u:'privacy.kisa.or.kr',t:'(국번없이) 118'},ag_spo:{n:'대검찰청',u:'www.spo.go.kr',t:'(국번없이) 1301'},ag_police:{n:'경찰청',u:'ecrm.police.go.kr',t:'(국번없이) 182'}};
  const activeAgencies=Object.keys(S.agency).filter(k=>S.agency[k]);

  // Icons
  const icons=[{e:'📋',l:'개인정보 수집'},{e:'🎯',l:'처리 목적'},{e:'🗓️',l:'보유 기간'},{e:'🤝',l:'처리 위탁'},{e:'🔗',l:'제3자 제공'},{e:'📨',l:'고충 처리'},{e:'🌏',l:'국외 이전'},{e:'🔒',l:'안전 조치'}];
  const iconColors=['#4f6ef7','#7c5ce7','#00b894','#e17055','#fdcb6e','#0984e3','#6c5ce7','#00cec9'];
  ...
  const numMap = {};
  visibleToc.forEach((t,i) => { numMap[t.k] = String(i+1).padStart(2,'0'); });
  // Helper: render section heading (skips if key not in numMap)
  const sec = (k, label, opt=false) => {
    if(!numMap[k]) return '';
    return '<div id="pp-'+k+'" class="pp-sec"><div class="pp-sec-num">'+numMap[k]+'</div>'+label+(opt?' <span style="font-size:11px;color:#aaa;font-weight:400;">[해당시]</span>':'')+'</div>';
  };

  return `
<h2 class="pp-h2">${co} 개인정보 처리방침</h2>
...
<div class="pp-toc-box">
  <ul>${visibleToc.map((t,i)=>{const n=numMap[t.k];return `<li><a href="#pp-${t.k}" class="pp-toc-link"><span class="pp-toc-num">${n}</span>${t.l}${t.opt?'<span class="pp-toc-opt">[해당시]</span>':''}</a></li>`;}).join('')}</ul>
</div>

${sec("collect","개인정보의 처리 목적, 처리 항목, 보유 및 이용기간")}
<p>${alias}는 「개인정보 보호법」에 따라 서비스 제공을 위해 필요 최소한의 범위에서 개인정보를 수집·이용합니다.</p>

${(S.collectNoConsent.length>0&&S.collectNoConsent.some(r=>r.category||r.items))?`
...
${(S.collectConsent.length>0&&S.collectConsent.some(r=>r.category||r.items))?`
...
<tbody>${S.dlItems.map(r=>`<tr><td class="c">${r.company||'-'}</td><td>${r.task||'-'}</td></tr>`).join('')}</tbody></table>`:'<p style="color:#aaa;font-style:italic;font-size:12px;">수탁업체를 추가해 주세요.</p>'}
...
<tbody>${S.otItems.map(r=>`<tr><td>${r.receiver||'-'}</td><td class="c">${r.country||'-'}</td><td>${r.items||'-'}</td><td>${r.purpose||'-'}</td><td>${r.method||'-'}</td><td class="c">${r.retention||'-'}</td></tr>`).join('')}</tbody></table>`:'<p style="color:#aaa;font-style:italic;font-size:12px;">이전 대상을 추가해 주세요.</p>'}
`:''}
...
<tbody>${S.bhItems.map(r=>`<tr><td>${r.items||'-'}</td><td>${r.purpose||'-'}</td><td>${r.method||'-'}</td><td class="c">${r.retention||'-'}</td></tr>`).join('')}</tbody></table>`:'<p style="color:#aaa;font-style:italic;font-size:12px;">행태정보 항목을 추가해 주세요.</p>'}
`:''}
...
<div class="pp-contact-box">
  ...
  <div class="pp-contact-info">
    ${S.depts.filter(d=>d.name||d.email||d.phone).map(d=>`
      ${d.name?'부서: '+d.name:''} ${d.phone?'&nbsp;|&nbsp; 전화: '+d.phone:''} ${d.email?'&nbsp;|&nbsp; 이메일: '+d.email:''}
    `).join('<br>') || '<span class="pp-placeholder">부서 정보를 입력해 주세요</span>'}
  </div>
</div>
...
function copyHTML(){
  navigator.clipboard.writeText(generateFinalHTML()).then(()=>showToast('📋 HTML 코드가 복사되었습니다!','success'));
}
