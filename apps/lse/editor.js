(function () {
  ...
  function loadBothPanels(html, baseUrl, statusMsg) {
    ...
    setOrigSrcdoc(injectScrollSync(html, baseUrl));
  ...
  function setupEditorFrame(html, baseUrl) {
    ...
    setEditSrcdoc(stripScripts(html));
  }

  /* ── srcdoc 헬퍼 (Blob URL 불필요 — 내부망/보안 환경 호환) ── */
  function setOrigSrcdoc(html) {
    if (_origBlobUrl) { URL.revokeObjectURL(_origBlobUrl); _origBlobUrl = null; }
    frame.removeAttribute("src");
    frame.srcdoc = html;
  }

  function setEditSrcdoc(html) {
  ...
  origBody.addEventListener("drop", function (e) {
    ...
    if (!file) return;
    if (!/\.html?$/i.test(file.name)) { alert("HTML(.html, .htm) 파일만 업로드할 수 있습니다."); return; }
    var reader = new FileReader();
    reader.onload = function (ev) {
